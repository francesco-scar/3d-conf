...
void MarlinUI::set_custom_characters(const HD44780CharSet screen_charset/*=CHARSET_INFO*/) {
  ...
  // CHARSET_BOOT
    ...
    const static PROGMEM byte heart[8] =  { // BEFORE-DEFAULT ADDED
      B00000,
      B01010,
      B11111,
      B11111,
  ...
  const static PROGMEM byte clock[8] = {
    ...
    if (screen_charset == CHARSET_BOOT) {
      ...
      createChar_P(5, heart);					// BEFORE-DEFAULT ADDED
    }
    else
  #endif
...
void MarlinUI::clear_lcd() { lcd.clear(); }
  ...
  static void logo_lines(PGM_P const extra) {
  	lcd.clear();
  	lcd_put_u8str_P(PSTR( "     " ));	// BEFORE-DEFAULT ADDED
  	char boot_str1[] = "Made with ";
  	for (short i = 0; i < 10; i++){
  		lcd.write(boot_str1[i]);
  		delay(150);
  	}
  	lcd.write(5);
  	lcd.setCursor(0,2);
  	lcd_put_u8str_P(PSTR( "     " ));	// BEFORE-DEFAULT ADDED
  	char boot_str2[] = "Powered by";
  	for (short i = 0; i < 10; i++){
  		lcd.write(boot_str2[i]);
  		delay(150);
  	}
  	lcd.setCursor(0,3);
  	lcd_put_u8str_P(PSTR( "  " ));	// BEFORE-DEFAULT ADDED
  	char boot_str3[] = "Open-Source Code";
  	for (short i = 0; i < 16; i++){
  		lcd.write(boot_str3[i]);
  		delay(150);
  	}
  	delay(5000);	// BEFORE-DEFAULT ADDED
  	lcd.clear();	// BEFORE-DEFAULT ADDED
    ...
    delay(3000);	// BEFORE-DEFAULT ADDED
  }
  ...
  void MarlinUI::show_bootscreen() {
    ...
    if (LCD_EXTRA_SPACE >= utf8_strlen(SHORT_BUILD_VERSION) + 1) {
      ...
      CENTER_OR_SCROLL(MARLIN_WEBSITE_URL, 6000);	// BEFORE-DEFAULT CENTER_OR_SCROLL(MARLIN_WEBSITE_URL, 2000);
    }
    else {
      //
